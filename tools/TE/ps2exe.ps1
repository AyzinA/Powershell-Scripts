...
"@ })
    ...
    {
$(if ($noConsole){ @"
        // Speicher fÃ¼r Konsolenfarben bei GUI-Output werden gelesen und gesetzt, aber im Moment nicht genutzt (for future use)
...
"@ } else {@"
                // Dummywert fÃ¼r Winforms zurÃ¼ckgeben.
...
$(if ($noConsole){ @"
    ...
    public class Choice_Box
    {
        public static int Show(System.Collections.ObjectModel.Collection<ChoiceDescription> arrChoice, int intDefault, string strTitle, string strPrompt)
        {
            ...
            if (!string.IsNullOrEmpty(strPrompt))
            {
                ...
                label.MaximumSize = new System.Drawing.Size(System.Windows.Forms.Screen.FromControl(form).Bounds.Width*5/8 - 18, 0);
                label.AutoSize = true;
                // erst durch Add() wird die GrÃ¶ÃŸe des Labels ermittelt
                form.Controls.Add(label);
                iPosY = label.Bottom;
                iMaxX = label.Right;
            }
 
            // An den Radiobuttons orientieren sich die weiteren GrÃ¶ÃŸen und Positionen
    ...
    public class ReadKey_Box
    {
        ...
        public static KeyInfo Show(string strTitle, string strPrompt, bool blIncludeKeyDown)
        {
            ...
            // Am Label orientieren sich die GrÃ¶ÃŸen und Positionen
...
"@ } else {@"
            ...
            // Titel und Labeltext fÃ¼r Input_Box zurÃ¼cksetzen
            ib_caption = "";
            ib_message = "";
"@ })
...
Invoke-ps2exe -inputFile "mini.ps1" -outputFile "Tools Mini.exe" -iconFile "RDP.ico" -noConsole -noError -title "Tools Mini by Alexander Ayzin" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#Invoke-ps2exe -inputFile SendMails.ps1 -outputFile SendMails.exe -iconFile SendMails.ico -noConsole -noError -title "SendMails" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#Invoke-ps2exe -inputFile "Active IDM.ps1" -outputFile "Active IDM.exe" -iconFile "Active IDM.ico" -noConsole -noError -title "Active IDM" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#Invoke-ps2exe -inputFile "SMS.ps1" -outputFile "SMS.exe" -iconFile SMS.ico -noConsole -noError -requireAdmin -title "SMS" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#Invoke-ps2exe -inputFile "SMS - rabin.ps1" -outputFile "SMS - rabin.exe" -iconFile SMS.ico -noConsole -noError -requireAdmin -title "SMS - Rabin" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#Invoke-ps2exe -inputFile PIGUA.ps1 -outputFile a.exe -noConsole -noError -requireAdmin -title "ôéâåò" -copyright "© àìëñðãø àééæéï" -version 1.0.0.0
#
pause