...
$btn_Opt1.Add_Click({if($viewForm.working -eq $true){return};$viewForm.working = $true;$Form.Height = 278;

Write-Host $btn_Opt1.Name

if($i_User.Text -eq ""){$Data_Output_Label.Visibility = "Visible";$Data_Output_Label.Content = "חסר פרטי משתמש";$viewForm.working = $false;return};CLS

$Data_Output_Label.Visibility = "Visible"
$Data_Output_Label.Content = "LOADING DATA FROM SERVER..."
if($Form.Height -ge 277 -and $Form.Height -le 277.5){$Form.Height = $Form.Height +1}else{$Form.Height = 277}
$Data_Output_Label.Visibility = "Hidden"
...
$Form.Height = 450;$Data_Output_Label.Visibility = "Hidden";$viewForm.working = $false})

$btn_Opt2.Add_Click({if($viewForm.working -eq $true){return};$viewForm.working = $true;$Form.Height = 277;

Write-Host $btn_Opt2.Name

if($i_Computer.Text -eq ""){$Data_Output_Label.Visibility = "Visible";$Data_Output_Label.Content = "חסר פרטי מחשב";$viewForm.working = $false;return};CLS
...
Debug.log("גירסת מערכת הפעלה:"+(Get-WmiObject -ClassName win32_operatingsystem -ComputerName $ip).OSArchitecture)
if($ip){
...
$UserPersonalDir.Text = $pc_data_current_user.HomeDirectory
$UserOUDir.Text = $pc_data_current_user.CanonicalName

}}

$Form.Height = 450;$Data_Output_Label.Visibility = "Hidden";$viewForm.working = $false})

$btn_Opt3.Add_Click({if($viewForm.working -eq $true){return};$viewForm.working = $true;$Form.Height = 278;

Write-Host $btn_Opt3.Name
